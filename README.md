...
- 강한 참조를 막는 방법
    1. weak
        1. 가르키는 대상이 메모리에서 해제가 되면 자동으로 nil이 됨
        2. 변수를 선언할 때 무조건 옵셔널타입으로 설정해야됨
        3. 보통 사용
    2. unowned
        1. 가르키는 대상이 메모리에서 해지가 되도 대상의 메모리 주소를 가지고 있음
        2. 옵셔널 타입으로 설정 X → 문법이 조금 바뀜(불편 ex. 초기값할당 등)
        3. 가르키는 상대방이 메모리에서 먼저 해제가 되지 않는 것이 명확할때
        4. 댕글링 포인터 
    3. 클래스와 클로저가 서로 가르키는 경우
        1. 클로저 안에 캡쳐리스트 `[weak self]` 적용
        2. 캡처리스트 → closure 안에서 외부의 변수들을 캡처할 때 strong, weak, unowneded 등의 참조 강도를 명시해서 캡처해오는 방법이다.
