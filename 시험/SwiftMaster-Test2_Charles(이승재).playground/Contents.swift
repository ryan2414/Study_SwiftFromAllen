...
// 함수의 실행
var maxN = findMaxNumber(array: numArray)
print(maxN)
...
// 1.break문
// 반복되는 스코프를 끝내고 나올 때, switch 문에서 특정 case를 끝낼때 사용한다.


// 2.fallthrough문
// switch문에서 다음 case까지 조건을 보지 않고 실행할 때 사용한다.


// 3.continue문
// 반복문에서 해당 인자는 확인하지 않고 넘어간다


// 4.return문
// 함수에서 특정 값을 반환하고, 리턴타입이 없는 경우 함수를 종료할때 사용한다.
...
// 2번 방법
// 조건문으로 nil이 아니라는 것을 확인 후, 강제로 언래핑
...
switch login1 {
case let .google(email: email, password: password):
    print("이메일 주소: \(email), 비밀번호: \(password)")
case let .facebook(email: email, password: password):
    print("이메일 주소: \(email), 비밀번호: \(password)")
case let .naver(email: email, password: password):
    print("이메일 주소: \(email), 비밀번호: \(password)")
}
...
// 고유의 메모리 공간을 가지는 값이 저장되는 일반적인 속성(변수)을 저장 속성이라고 한다.
...
// 클래스는 데이터 영역에 생성되어 인스턴스 시 힙 영역에 생성, 상속이 가능하다.
// 구조체는 인스턴스 시 스택 영역에 생성, 상속이 불가능하다.
