...
// 오버로딩과 오버라이딩
/**
 - 오버로딩 - 함수에서 함수의 하나의 이름에 여러 함수를 대응시켜서 사용
 - 오버라이딩 - 클래스의 상속에서 상위클래스의 속성/메서드를 재정의(기능을 약간 변형하여 사용)하는 것
...
/**
 ...
 - --> 속성 감시자를 추가하는 기능(메서드) (실질적 단순 메서드를 추가해서 저장 속성이 변하는 시점을 관찰할 뿐)
 
 - (2) 계산 속성(메서드)의 재정의
 - (실질적인 메서드이기 때문에 메서드 형태로의 재정의만 가능. 기능의 범위를 축소하는 형태로의 재정의는 불가능)
 
 - (상위) 읽기 전용 계산 속성을 --> (하위) 읽기/쓰기가 가능한 속성으로 재정의 가능(확장 O)
 - ----> 속성 감시자를 추가하는 재정의는 불가능 (읽기 전용 속성을 관찰할 수 없음 - 논리에 안 맞음)
 - (상위) 읽기/쓰기 계산 속성을 --> (하위) 읽기만 가능한 속성으로 재정의 불가능(기능 제한X)
 ...
 - 인스턴스 속성의 대원칙
 - 1) 저장 속성 재정의는 원칙적으로 불가능(하위 속성의 고유의 메모리 공간을 변형 불가). 메서드 방식(계산 속성으로의 재정의 가능)으로 추가 가능
 ...
 - (실질적으로 드뭄)
 - 타입 속성의 재정의 원칙
 - 1) 타입 저장 속성은 재정의 불가- static 키워드( 계산속성으로 재정의 하거나, 속성 감시자를 추가하는 것도 불가능)
...
/**
 ...
 - 상위 기능을 무시하고 새롭게 구현하는 것도 가능(제약 없음 - 메서드 이름만 동일하고 완전히 새롭게 구현 가능하다고 생각하면 됨)
 
 - 다만, 기능을 추가하는 구현을 선택할 시에 상위구현의 기능을 먼저 실행할지의 여부는 개발자의 선택
 */
...
//    }
    ...
    // 2) 저장속성을 속성감시자를 추가하는 재정의
    override var currentSpeed: Double {
        willSet {
            print("값이 \(currentSpeed)에서 \(newValue)로 변경 예정")
        }
        didSet {
            print("값이 \(oldValue)에서 \(currentSpeed)로 변경 됨")
        }
    }
    
    // 계산속성을 재정의 가능 (super키워드 주의)
...
//    }
    ...
    // 계산속성을 재정의 하면서, 속성감시자 추가 가능 (속성감시자 부분 참고)
    override var halfSpeed: Double {
        willSet {
            print("값이 \(halfSpeed)에서 \(newValue)로 변경 예정")
        }
        didSet {
            print("값이 \(oldValue)에서 \(halfSpeed)로 변경 ")
        }
    }
}
...
/**
 - 재정의 - 현재 클래스에 맞게 상위 멤버를 변형시켜서, 사용하는 것
 
 - 대원칙:
 - 1) 저장 속성 재정의 불가
 - 2) 메서드는 자유롭게 재정의 가능(다만, 기능 확장만 가능 - 기능 축소 불가 의미)
 
 - (재정의를 하지 않아도, 상속에서는 당연히 모든 멤버의 상속이 일어남)
 
 - 저장 속성은 실제 인스턴스에 각각 멤버별(속성별로) 저장공간이 있고, 변형하는 것이 불가 (상속의 기본 원칙)
